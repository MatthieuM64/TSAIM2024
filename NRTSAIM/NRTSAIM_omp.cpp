...
/*NON-RECIPROCAL TWO SPECIES ACTIVE ISING MODEL - CONSTANT SPECIES POPULATION*/
...
void exportProfiles(const vector<double> &RHOA, const vector<double> &RHOB, const vector<double> &MAGA, const vector<double> &MAGB, const double &beta, const double &D, const double &v, const double &theta, const double &rho0, const double &mag0, const double &JAB, const double &JBA, const int &LX, const int &LY, const int &init, const int &RAN, const int &t)
{
	//Creation of the file.
	int returnSystem=system("mkdir -p data_NRTSAIM_dynamics1d/");
	stringstream ss;
	ss << "./data_NRTSAIM_dynamics1d/NRTSAIM_profile_beta=" << beta << "_D=" << D << "_v=" << v << "_theta=" << theta << "_rho0=" << rho0 << "_mag0=" << mag0 << "_JAB=" << JAB << "_JBA=" << JBA << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";
...
//Export densities.
void exportDensity(const vector< vector<int> > &RHOA, const vector< vector<int> > &RHOB, const double &beta, const double &D, const double &v, const double &theta, const double &rho0, const double &mag0, const double &JAB, const double &JBA, const int &LX, const int &LY, const int &init, const int &RAN, const int &t)
{
	//Creation of the file.
	int returnSystem=system("mkdir -p data_NRTSAIM_dynamics2d/");
	stringstream ssDensity;
	ssDensity << "./data_NRTSAIM_dynamics2d/NRTSAIM_density_beta=" << beta << "_D=" << D << "_v=" << v << "_theta=" << theta << "_rho0=" << rho0 << "_mag0=" << mag0 << "_JAB=" << JAB << "_JBA=" << JBA << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";
...
void ReadCommandLine(int argc, char** argv, double &beta, double &D, double &v, double &theta, double &rho0, double &mag0, double &JAB, double &JBA, int &LX, int &LY, int &init, int &tmax, int &RAN, int &THREAD_NUM)
{
...
{
	//Physical parameters: beta=inverse temperature, D=diffusion, v=self-propulsion velocity, rho0=average density, mag0=average species magnetization (divided by rho0), (JAB,JBA)=non-reciprocal couplings, LX*LY=size of the box.
	double beta=1.25, D=1., v=1.8, theta=0., rho0=4., mag0=0., JAB=1., JBA=-1.;
	int LX=1024, LY=128;
	...
	ReadCommandLine(argc,argv,beta,D,v,theta,rho0,mag0,JAB,JBA,LX,LY,init,tmax,RAN,THREAD_NUM);
	...
	ssAverages << "./data_NRTSAIM_averages/NRTSAIM_averages_beta=" << beta << "_D=" << D << "_v=" << v << "_theta=" << theta << "_rho0=" << rho0 << "_mag0=" << mag0 << "_JAB=" << JAB << "_JBA=" << JBA << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << ".txt";
	...
	//Time increment.
	const double dt=1./(4*D+theta*v+exp(2*beta));
	
	//Probability to hop.
	const double proba_hop=(4*D+theta*v)*dt;
	
	//Coupling constants.
	vector< vector<double> > Jcoup(2,vector<double>(2,1.));
	Jcoup[0][1]=JAB;
	Jcoup[1][0]=JBA;
	cout << "#####################" << endl;
	cout << "Coupling constant:" << endl;
	cout << Jcoup[0][0] << " " << Jcoup[0][1] << endl;
	cout << Jcoup[1][0] << " " << Jcoup[1][1] << endl;
	cout << "#####################" << endl;
	...
	for(int t=0;t<=tmax;t++)
	{
		...
		{
			//Export 2d snapshots.
			exportDensity(RHOA,RHOB,beta,D,v,theta,rho0,mag0,JAB,JBA,LX,LY,init,RAN,t);
			...
			exportProfiles(rhoA,rhoB,magA,magB,beta,D,v,theta,rho0,mag0,JAB,JBA,LX,LY,init,RAN,t);
			...
			cout << "time=" << t << " -rhoA=" << nA << " -rhoB=" << nB << " -mA=" << mA << " -mB=" << mB << running_time.TimeRun(" ") << endl;
			fileAverages <<  t << " " << nA << " " << nB << " " << mA << " " << mB << endl;
		}
		...
		#pragma omp parallel
		{
			...
			for (int i=0; i<Nth; i++)
			{
				...
				//Probability to flip the spin (non reciprocal).
				omp_set_lock(&lock_site[X0][Y0]);
				const int magA=MAGA[X0][Y0], magB=MAGB[X0][Y0], rho=RHOA[X0][Y0]+RHOB[X0][Y0];
				omp_unset_lock(&lock_site[X0][Y0]);
				const double proba_spin_flip=exp(-2*beta*spin*double(Jcoup[species][0]*magA+Jcoup[species][1]*magB)/double(rho))*dt;
				const double ptot=proba_hop+proba_spin_flip;
