...
/*TWO SPECIES ACTIVE ISING MODEL - CONSTANT SPECIES POPULATION*/
...
void exportProfiles(const vector<double> &RHOA, const vector<double> &RHOB, const vector<double> &MAGA, const vector<double> &MAGB, const double &beta, const double &D, const double &v, const double &theta, const double &rho0, const double &mag0, const int &LX, const int &LY, const int &init, const int &RAN, const int &t)
{
	//Creation of the file.
	int returnSystem=system("mkdir -p data_TSAIM_dynamics1d/");
	stringstream ss;
	ss << "./data_TSAIM_dynamics1d/TSAIM_profile_beta=" << beta << "_D=" << D << "_v=" << v << "_theta=" << theta << "_rho0=" << rho0 << "_mag0=" << mag0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";
...
//Export densities.
void exportDensity(const vector< vector<int> > &RHOA, const vector< vector<int> > &RHOB, const double &beta, const double &D, const double &v, const double &theta, const double &rho0, const double &mag0, const int &LX, const int &LY, const int &init, const int &RAN, const int &t)
{
	//Creation of the file.
	int returnSystem=system("mkdir -p data_TSAIM_dynamics2d/");
	stringstream ssDensity;
	ssDensity << "./data_TSAIM_dynamics2d/TSAIM_density_beta=" << beta << "_D=" << D << "_v=" << v << "_theta=" << theta << "_rho0=" << rho0 << "_mag0=" << mag0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";
...
void ReadCommandLine(int argc, char** argv, double &beta, double &D, double &v, double &theta, double &rho0, double &mag0, int &LX, int &LY, int &init, int &tmax, int &RAN, int &THREAD_NUM)
{
...
{
	//Physical parameters: beta=inverse temperature, D=diffusion, v=self-propulsion velocity, theta=model parameter, rho0=average density, mag0=average species magnetization (divided by rho0), LX*LY=size of the box.
	double beta=0.75, D=1., v=1.8, theta=0., rho0=5., mag0=0.;
	int LX=1024, LY=128;
	...
	ReadCommandLine(argc,argv,beta,D,v,theta,rho0,mag0,LX,LY,init,tmax,RAN,THREAD_NUM);
	...
	ssAverages << "./data_TSAIM_averages/TSAIM_averages_beta=" << beta << "_D=" << D << "_v=" << v << "_theta=" << theta << "_rho0=" << rho0 << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << ".txt";
	...
	for(int t=0;t<=tmax;t++)
	{
		...
		{
			//Export 2d snapshots.
			exportDensity(RHOA,RHOB,beta,D,v,theta,rho0,mag0,LX,LY,init,RAN,t);
			...
			exportProfiles(rhoA,rhoB,magA,magB,beta,D,v,theta,rho0,mag0,LX,LY,init,RAN,t);
		...
		#pragma omp parallel
		{
			...
			for (int i=0; i<Nth; i++)
			{
				//Choose a particle randomly (j) at site (X0,Y0).
				...
				const int rho=RHOA[X0][Y0]+RHOB[X0][Y0], va=MAGA[X0][Y0]-MAGB[X0][Y0];
				omp_unset_lock(&lock_site[X0][Y0]);
				const double proba_spin_flip=exp(-2*beta*spin*speciesSpin*double(va)/double(rho))*dt;
